...
$disallowed_patterns = ['*.php','*.asp'];  // must be an array.  Matching files not allowed to be uploaded
...
th {font-weight: normal; color: rgb(10,10,10); background-color: #03dac5; padding:.5em 1em .5em .2em;
...
<?php endif; ?>
	...
	function renderFileRow(data) {
		...
		var $dl_link = $('<a/>').attr('href', encodeURIComponent(data.path)).addClass('download').text('download').attr("download", data.name);
...
<!-- JeremiahV8 Additions -->
<?php
	$statistics = '../statistics.php';
	if (file_exists($statistics)) {
		include $statistics;
	}else{
		echo "Err";
	}
